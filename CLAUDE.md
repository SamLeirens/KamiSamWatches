...
- **Backup**: `BackupService` (`Services/BackupService.swift`) encodes/decodes a versioned JSON backup (`BackupFile` DTOs in `Models/BackupFile.swift`, ISO8601 dates); `DataStore.restore(shows:events:)` merges it in, deduping events on show/season/episode + whole-second timestamp so rewatches survive and re-imports are no-ops.
- **CloudKit readiness (sync not enabled)**: both `@Model` classes keep default values on every stored property and use no unique constraints; the container in `Kami_Sam_WatchesApp` is an explicit `ModelConfiguration(schema:cloudKitDatabase: .none)`. To enable iCloud sync later (paid Apple Developer account required): add the iCloud capability with a CloudKit container + Background Modes → Remote notifications in Signing & Capabilities, then change `.none` to `.automatic`. Existing local data uploads on first sync. Keep new model properties defaulted/optional per AGENTS.md.
...
| **Stats** | `Stats/` | `StatsViewModel` (owned by `StatsView`) computes `watchTimeLabel` (mo/d/h/m) and `monthlyActivity` ([MonthlyActivity]) for a 12-month zero-filled bar chart. 2×2 metric tile grid + Swift Charts `BarMark` activity chart + watch-event history list. Toolbar menu hosts **Import from TV Time** (`.fileImporter` for a ZIP → `TVTimeImporter`) and **Export/Import Backup** (JSON via `BackupService` + `BackupDocument` `.fileExporter`/`.fileImporter` → `DataStore.restore`). |
